...
class model_evaluate:
    def __init__(self, df_predict, true_pick, target_phase, positive_pick,
        ...
        if self.target_phase.lower() == 'p':
            if self.dataset_type=="STEAD":
                self.df_fail_idx = np.logical_and(
                self.df_predict["manP"].values>0, 
                self.df_predict["predP"].values<0)

                self.df_predict_suc = self.df_predict[np.logical_and(
                    self.df_predict["manP"].values>0, 
                    self.df_predict["predP"].values>0)]
            else:
                self.df_fail_idx = np.array(self.df_predict.predS.values < 0)
                self.df_predict_suc = self.df_predict[
                    self.df_predict.predS.values >= 0]

            self.df_TruePositive_idx = np.logical_and(
                np.array( np.fabs(self.df_predict_suc.manP-self.df_predict_suc.predP)
                          <= self.true_pick),
                np.array(self.df_predict_suc.predP_prob >= self.positive_pick ))
            ...
            self.df_TruePositive = self.df_predict_suc[self.df_TruePositive_idx]
            self.df_TrueNegative = self.df_predict_suc[self.df_TrueNegative_idx]
            self.df_FalsePositive = self.df_predict_suc[self.df_FalsePositive_idx]
            self.df_FalseNegative = self.df_predict_suc[self.df_FalseNegative_idx]


        elif self.target_phase.lower() == 's':
            if self.dataset_type=="STEAD":
                self.df_fail_idx = np.logical_and(
                self.df_predict["manS"].values>0, 
                self.df_predict["predS"].values<0)
            ...
            self.df_TruePositive_idx =  np.logical_and(
                np.array(np.fabs(self.df_predict_suc.manS-self.df_predict_suc.predS)
                     <= self.true_pick),
                np.array(self.df_predict_suc.predS_prob >= self.positive_pick))
        ...
        self.TrueNegative = len(self.df_TrueNegative) 
        self.FalsePositive = len(self.df_FalsePositive)
        self.FalseNegative = len(self.df_FalseNegative) +\
                             len(df_predict[self.df_fail_idx])

        assert self.TruePositive + self.TrueNegative + self.FalsePositive +\
               self.FalseNegative == len(self.df_predict["manP"].values>0)
    ...
    def picking_rate(self):
        try:
            return self.TruePositive / self.df_length
        except:
            return 0
        ...
        try:
            return (self.TruePositive+self.TrueNegative)/ \
                      (self.TruePositive+self.TrueNegative+
                       self.FalsePositive+self.FalseNegative)
        except:
            return 0
    ...
    def AUROC(self, prob_thre=np.arange(0.05, 1.05, 0.05)):
        ...
        fprs = [model_evaluate(self.df_predict, self.true_pick,
          self.target_phase, positive_pick=i).FalsePositive_rate()
          for i in prob_thre]
        recalls = [model_evaluate(self.df_predict, self.true_pick,
             self.target_phase, positive_pick=i).recall()
             for i in prob_thre]
