...
class unets:
    ...
    def upconv_unit(self, inputs, nb_filter, 
                concatenate_layer, apply_attention=False):
    ...
    def build_unet(self, pretrained_weights=None,
        ...
        conv_init_exp = self.conv_unit(inputs=inputs,
        ...
        #========== Encoder
        down1 = self.conv_unit(inputs=conv_init_exp,
            nb_filter=nb_filters[0], stride_size=self.stride_size) # downsample
        down1_exp = self.conv_unit(inputs=down1,
            nb_filter=nb_filters[1], stride_size=None)
        # (None, 501, nb_filter[0])
        down2 = self.conv_unit(inputs=down1_exp,
            nb_filter=nb_filters[1], stride_size=self.stride_size)
        down2_exp = self.conv_unit(inputs=down2,
            nb_filter=nb_filters[2], stride_size=None)
        # (None, 126, nb_filter[1])
        down3 = self.conv_unit(inputs=down2_exp,
            nb_filter=nb_filters[2], stride_size=self.stride_size)
        down3_exp = self.conv_unit(inputs=down3, 
            nb_filter=nb_filters[3], stride_size=None)
        # (None, 32, nb_filter[2])

        #========== Decoder
        up4 = self.upconv_unit(inputs=down3_exp, 
            nb_filter=nb_filters[2], concatenate_layer=down2_exp)
        up4_fus = self.conv_unit(inputs=up4, 
            nb_filter=nb_filters[2], stride_size=None)

        up5 = self.upconv_unit(inputs=up4_fus, 
            nb_filter=nb_filters[1], concatenate_layer=down1_exp)
        up5_fus = self.conv_unit(inputs=up5, 
            nb_filter=nb_filters[1], stride_size=None)

        up6 = self.upconv_unit(inputs=up5_fus, 
            nb_filter=nb_filters[0], concatenate_layer=conv_init_exp)
        up6_fus = self.conv_unit(inputs=up6, 
            nb_filter=nb_filters[0], stride_size=None)

        ##========== Output map
        outmap = Conv1D(3, 1, 
            kernel_initializer=self.kernel_init)(up6_fus)#, kernel_regularizer=l2(1e-3)
        outmap = Activation(self.out_activation)(outmap)
        model = Model(inputs=inputs, outputs=outmap)
    ...
    def build_R2unet(self, pretrained_weights=None, 
            input_size=None, nb_filters=None):
        if nb_filters == None:
            nb_filters = self.nb_filters
        if input_size == None:
        ...
        up4 = self.upconv_unit(inputs=down3_exp, 
            nb_filter=nb_filters[2], concatenate_layer=down2_exp)
        up4_fus = self.RRconv_unit(inputs=up4, 
            nb_filter=nb_filters[2], stride_size=None)

        up5 = self.upconv_unit(inputs=up4_fus, 
            nb_filter=nb_filters[1], concatenate_layer=down1_exp)
        up5_fus = self.RRconv_unit(inputs=up5, 
            nb_filter=nb_filters[1], stride_size=None)

        up6 = self.upconv_unit(inputs=up5_fus, 
            nb_filter=nb_filters[0], concatenate_layer=conv_init_exp)
        up6_fus = self.RRconv_unit(inputs=up6, 
            nb_filter=nb_filters[0], stride_size=None)

        ##========== Output map
        outmap = Conv1D(3, 1, 
            kernel_initializer='he_uniform')(up6_fus)#, kernel_regularizer=l2(1e-3)
        outmap = Activation(self.out_activation)(outmap)
        model = Model(inputs=inputs, outputs=outmap)
    ...
    def build_attunet(self, pretrained_weights=None,
            input_size=None, nb_filters=None):
        if nb_filters == None:
            nb_filters = self.nb_filters
        if input_size == None:
