...
mdl_dir = os.path.join(basepath, 
    'pretrained_model/multitask_ARRU_20s')
...
#    custom_objects={'loss':loss_estimator})
...
for i in range(split_iter):
    ...
    for k in range(n_pred):
        ...
        trigger_mask = trigger_onset(
            smooth(pred_eq_mask, 10), 0.3, 0.3)
        ...
        try:
            ...
            p_peak, p_value = pick_peaks(predict_P, 
                    labeled_P, dt, search_win=1)
            s_peak, s_value = pick_peaks(predict_S, 
                    labeled_S, dt, search_win=1)
        except: 
        ...
        if len(trigger_mask) == 0:
            ...
            mask_pb = np.round(np.mean(pred_eq_mask[
                trigger_mask[0][0]:trigger_mask[0][1]]), 3)
        ...
        message = f"{evid}, {sta_info:>10s}, {chn:>3s}, "+\
                  ...
                  f"{label_type}, {pred_type}, {mask_pb:>5.2f}\n"
        f_out.write(message)
